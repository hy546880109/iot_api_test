...
class TestUserApiByTDD(unittest.TestCase):
    ...
    def get_token():
        ...
        args = (TestUserApiByTDD.device_sn, TestUserApiByTDD.os_platform, TestUserApiByTDD.app_version)
        content = ''.join(args).encode('ascii')
        sign_key = TestUserApiByTDD.SECRET_KEY.encode('ascii')
        sign = hmac.new(sign_key, content, hashlib.sha1).hexdigest()
        data = {'sign': sign}
        response = TestUserApiByTDD.http.post(uri, data=json.dumps(data), headers=headers)
        print(response.text)
        token = response.json().get('token')
        print(token)
        return token
    ...
    def test_002_query_all_users(self):
        '''测试查询所有用户'''
        uri = '/api/users'
        response = TestUserApiByTDD.http.get(uri, data=json.dumps(self.playload), headers=self.headers)
        print(response.text)
        count = response.json().get('count')
        items = response.json().get('items')
        self.assertEqual(response.status_code, 200, '请求返回非200')
